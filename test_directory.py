...
def test_not_directory_exception_caught():
    with when(os.path).isdir("/root_test/").thenReturn(False):
...
def test_found_file_with_word():
    ...
    with when(os.path).isdir("/root_test/").thenReturn(True):
        with patch("builtins.open", mock_open(read_data="Foo")) as mock_file:
