...
import training.file as file_path


def test_match_files__os_listdir_is_empty__no_fail():
...
def test_match_files__os_listdir_not_empty__no_fail():
    when(os.path).isdir('root').thenReturn(True)
...
def test_match_files__file_path_from_path_returns_none__no_fail():
    when(os.path).isdir('root').thenReturn(True)
    when(os.path).isdir("root/file0").thenReturn(False)
    when(os).listdir('root').thenReturn(["file0"])
...
def test_match_files__file_path_from_path_not_none__no_fail():
    when(os.path).isdir('root').thenReturn(True)
    when(os.path).isdir("root/file0").thenReturn(False)
    when(os).listdir('root').thenReturn(["file0"])
    mock_file0 = mock()
...
def test_match_files__file_only_in_root_directory__one_match():
    when(os.path).isdir('root').thenReturn(True)
    when(os.path).isdir("root/file0").thenReturn(False)
...
def test_match_files__file_in_sub_root_directory__one_match():
    when(os.path).isdir('root').thenReturn(True)
...
def test_match_files__two_files_in_directory_and_sub_root__two_matches():
    when(os.path).isdir('root').thenReturn(True)
    when(os.path).isdir('root/fold0').thenReturn(True)
    when(os.path).isdir("root/file0").thenReturn(False)
    when(os.path).isdir("root/fold0/file1").thenReturn(False)
