...
def test_search_by_content_existing_folder(files_scanner):
    word="hello"
...
def test_search_by_content_unexisting_folder(files_scanner):
    word = "hello"
    testing_folder_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "unexisted_folder")
    with pytest.raises(FolderNotFoundError):
        files_found = files_scanner.search_by_content(testing_folder_path, word)

def test_search_by_content_file_instead_of_folder(files_scanner):
    word = "hello"
